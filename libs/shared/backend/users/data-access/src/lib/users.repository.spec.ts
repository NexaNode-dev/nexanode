...
describe('UsersRepository', () => {
  ...
  describe('findOne', () => {
    it('should return a single user', async () => {
      const result = await provider.getUser({ where: { id: expectedUser.id } });
    ...
    it('should throw a NotFoundException if no user is found', async () => {
      ...
      try {
        await provider.getUser({ where: { id: expectedUser.id } });
      } catch (e) {
  ...
  describe('update', () => {
    it('should update a user with valid parameters', async () => {
      const updatedUser = await provider.updateUser(expectedUser.id, userData);
    ...
    it('should throw a NotFoundException if no user is found', async () => {
      ...
      try {
        await provider.updateUser(expectedUser.id, userData);
      } catch (e) {
        expect(e).toBeInstanceOf(NotFoundException);
      }
    });
  ...
  describe('delete', () => {
    it('should delete a user', async () => {
      const result = await provider.deleteUser(expectedUser.id);
    ...
    it('should throw a NotFoundException if no user is found', async () => {
      mockRepository.findOne.mockRejectedValueOnce(new NotFoundException());
      try {
        await provider.deleteUser(expectedUser.id);
      } catch (e) {
        expect(e).toBeInstanceOf(NotFoundException);
      }
    });
